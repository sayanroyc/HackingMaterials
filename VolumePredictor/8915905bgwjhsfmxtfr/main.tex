...
\title{\LARGE \bf
Unit Cell Volume Predictor (Pre-processing for \\
...
\author{Sayan Roychowdhury$^{1}$, Anubhav Jain$^{2}$% <-this % stops a space
...
Understanding how atoms are arranged in a material, and the implications for its properties of that arrangement, is central to modern science and technology. The discovery and development of new stronger, cheaper, lighter and more functional materials is important to maintain the steady technological progress to which we have become accustomed. It is increasingly recognized that theory and computation play a key role in materials discovery. Advances in electronic structure theory, improved software and substantial year-on-year improvement in computing power have combined to make accurate materials simulations not only possible, but almost routine.

The unparalleled success of density functional theory (DFT) in explaining materials phenomena has led to the concept of ‘materials by design’—the idea that for any given set of properties we can predict novel materials that outperform any of the known ‘usual suspects’. While achieving this lofty goal fully may be some time away, DFT calculations can now predict a significant portion of the properties for a given material. Given a crystal structure, it is straightforward to compute a wide range of mechanical, spectral, and thermodynamic properties.

Although successful, DFT is computationally expensive, requiring its calculations to be spread across several cores in order to be complete in a reasonable amount of time, even for a small data set. This paper explores one method of reducing this computation time. Using DFT requires an initial estimate of volume for the specified unit cell, and runs several iterations to optimize this value. Naturally, the worse the initial estimate, the more iterations it will take to converge, leading to a longer computation time. Our goal is to create a better initial estimate for the unit cell volume, which in turn will reduce the DFT computational time.


\section{Predicting Atomic Structure}
